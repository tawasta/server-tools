...
class AuthSAMLController(http.Controller):
    ...
    def slo_request(self, req, *args, **kwargs):
        ...
        client = provider._get_client_for_provider(request.httprequest.url_root.rstrip("/"))
        ...
        # LogoutRequest handling STARTS
            ...
            saml_token = request.env["auth_saml.token"].sudo().search([
...
class SAMLSession(Session):
    ...
    def logout(self, redirect="/web"):
        ...
        if request.session.get("_saml_user") and saml_token:
            ...
            idp_entity_id = re.search(r"\sentityID=\"(.*)\"", provider.idp_metadata).group(0)
            if not (service_location and idp_entity_id):
                _logger.error("Tried to SLO but didn't find IDP service and IDP entity ID")
