...
class ResUser(models.Model):
    ...
    def _auth_saml_signin(self, provider, validation, saml_response):
        ...
        _logger.error("HERE token_osv: " + str(token_osv))
        saml_uid = validation["user_id"]
        _logger.error("HERE saml_uid: " + str(saml_uid))
        prov = self.env["auth.saml.provider"].sudo().browse(provider)
        _logger.error("HERE prov: " + str(prov))
        if prov.use_hashed_id:
            # Use hashed value for detecting and saving, with dbsecret as salt
            db_secret = (
                self.env["ir.config_parameter"].sudo().get_param("database.secret")
            )
            saml_uid = hashlib.sha256(
                "{}{}".format(saml_uid, db_secret).encode("utf-8")
            ).hexdigest()
        ...
        _logger.error("HERE user: " + str(user))
        if len(user) == 0:
            ...
            _logger.error("HERE user (!= 1)" + str(user))
            raise AccessDenied()
        ...
        _logger.error("HERE token_ids: " + str(token_ids))
        # TODO: Save session info to somewhere - tokens -model?
        name_id = validation.get("name_id")
        _logger.error("HERE name_id: " + str(name_id))
        if token_ids:
            token_ids.write(
                {
        ...
        _logger.error("HERE validation: " + str(validation))
        _logger.error("HERE user.login: " + str(user.login))
        return user.login
