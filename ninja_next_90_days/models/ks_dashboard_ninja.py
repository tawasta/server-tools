...
class KsDashboardNinja(models.Model):
    ...
    ks_date_filter_selection = fields.Selection(
        selection_add=[
            ("l_custom_next_90_days", "Next 90 days"),
        ],
        ondelete={"l_custom_next_90_days": "set default"},
    )
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec, domain=None):
        ...
        if self.ks_date_filter_selection_2 == "l_custom_next_90_days":
            self.ks_date_filter_selection_2 = "l_custom"

        res = super(KsDashboardNinja, self).ks_convert_into_proper_domain(
            ks_domain, rec, domain
        )

        if self.ks_date_filter_selection == "l_custom" and self.next_90_days_selected:
            self.ks_date_filter_selection = "l_custom_next_90_days"

        if (
            self.ks_date_filter_selection_2 == "l_custom"
            and self.next_90_days_selected_2
        ):
            self.ks_date_filter_selection_2 = "l_custom_next_90_days"
