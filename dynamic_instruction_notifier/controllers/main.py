...
class PortalNotificationController(http.Controller):
    ...
    def get_portal_notification(self, xmlid):
        ...
        if view:
            instruction_message = request.env['instruction.message'].sudo().search([('view_id', '=', view.id)], limit=1)
    ...
    def get_instruction_message(self, model_name):
        ...
        instruction_message = InstructionMessage.search([('model_id.model', '=', model_name)], limit=1)
